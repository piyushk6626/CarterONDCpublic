...
inputDataresturant = ['name', 'dob', 'pancard', 'gst_number', 'address', 'business_name', 'trade_license', 'udyam', 'account_number', 'ifsc_code', 'upi_id', 'fssai_number', 'phoneNumber']

def store_data_resturant(kwargs):
    # print(kwargs)
    ...
    c = conn.cursor()
    c.execute('''CREATE TABLE IF NOT EXISTS resturant (
        name TEXT,
        dob TEXT,
        pancard TEXT,
        gst_number TEXT,
    ...
    c.execute("INSERT INTO resturant (name, dob, pancard, gst_number, address, business_name, trade_license, udyam, account_number, ifsc_code, upi_id, fssai_number, phoneNumber) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)", (name, dob, pancard, gst_number, address, business_name, trade_license, udyam, account_number, ifsc_code, upi_id, fssai_number, phoneNumber))
    conn.commit()
    conn.close()

def send_message_resturant(data_type):
    msg = ""
